...
import { Edit, Trash2, Sparkles, Loader2, Pencil } from "lucide-react";
            ...
            <DataTable
                ...
                columns={[
                    {
                        ...
                        cell: (row) => (
                            <div className="flex flex-col">
                                ...
                                <Badge variant="outline" className="w-fit mt-1 text-[10px] font-normal text-muted-foreground">{row.category}</Badge>
                                {row.description && (
                                    <span className="text-[10px] text-muted-foreground mt-1 line-clamp-2" title={row.description}>{row.description}</span>
                                )}
                            </div>
                        )
                    },
                        ...
                        cell: (row) => (
                            <div className="flex flex-wrap gap-1">
                                {row.attributes && Object.keys(row.attributes).length > 0 ? (
                                    Object.entries(row.attributes).slice(0, 4).map(([k, v]: any, i) => (
                                ...
                                {row.attributes && Object.keys(row.attributes).length > 4 && (
                                    <span className="text-[10px] text-muted-foreground">+{Object.keys(row.attributes).length - 4} more</span>
                                )}
                            </div>
                        )
                    },
                    {
                        ...
                        cell: (row) => (
                            row.skus && row.skus.length > 0 ? (
                                <div className="flex flex-wrap gap-1">
                                    {row.skus.slice(0, 3).map((sku: any, idx: number) => (
                                    ...
                                    {row.skus.length > 3 && (
                                        <span className="text-[10px] text-muted-foreground">+{row.skus.length - 3}</span>
                                    )}
                                </div>
                            ) : (
                                <div className="flex items-center">
                                    <Button
                                        ...
                                        onClick={() => onGenerateSKU(row)}
                                        disabled={generatingSKUId === row.id}
                                    >
                                        {generatingSKUId === row.id ? <Loader2 className="h-3 w-3 animate-spin" /> : "Auto-Gen SKU"}
                                    </Button>
                                </div>
                            )
                        )
                ...
                actions={(row) => (
                    <div className="flex justify-end gap-1">
                        <Button variant="ghost" size="icon" className="h-8 w-8 text-blue-600" onClick={() => onAiSuggest(row)} title="AI HSN Suggest">
